...
class Base_Entity extends Base {
	...
	{
		if($this->key == 1){
			$api = Server::getInstance()->getPluginManager ()->getPlugin ( "PDungeon" );
			foreach($api->db as $key => $val){
				foreach($val["pos"] as $key2 => $val2){
					if($val2["lev"] == $this->getLevel()->getFolderName()){
						$api->KillMob((string)$key);
						$api->Reset((string)$key);
					}
				}
			}
			$this->kill();
